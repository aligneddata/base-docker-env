docker run -it --rm \
  --net=host \
  -v "$(pwd):/workspace" \
  -v "$HOME/git/base-docker-env/pi/models.json.27b.32k:/root/.pi/agent/models.json" \
  -v "$HOME/git/base-docker-env/pi/settings.json.27b.32k:/root/.pi/agent/settings.json" \
  pi-agent \
  --provider ollama
